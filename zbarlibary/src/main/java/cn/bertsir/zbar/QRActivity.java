...
public class QRActivity extends Activity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        fl_title.setBackgroundResource(options.getTITLE_BACKGROUND_COLOR());
    ...
    private void fromAlbum() {
        ...
        if (QRUtils.getInstance().isMIUI()) {//是否是小米设备,是的话用到弹窗选取入口的方法去选取视频
            ...
            startActivityForResult(Intent.createChooser(intent, getResources().getString(R.string.qr_tips_album_fail)), REQUEST_IMAGE_GET);
        } else {//直接跳到系统相册去选取视频
            ...
            startActivityForResult(Intent.createChooser(intent, getResources().getString(R.string.qr_tips_album)), REQUEST_IMAGE_GET);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_IMAGE_GET && resultCode == RESULT_OK) {//从相册选取视频
            ...
            textDialog.setText(R.string.qr_waiting);
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        if(TextUtils.isEmpty(imagePath)){
                            Toast.makeText(getApplicationContext(), getResources().getString(R.string.qr_tips_album_fail), Toast.LENGTH_SHORT).show();
                            return;
                        }
                        //优先使用zbar识别
                            ...
                            public void run() {
                                if(!TextUtils.isEmpty(qrcontent)){
                                    ...
                                    if(!TextUtils.isEmpty(qrcontent)){
                                        ...
                                        Toast.makeText(getApplicationContext(), R.string.qr_tips_fail, Toast.LENGTH_SHORT).show();
                                        closeProgressDialog();
                                    }
